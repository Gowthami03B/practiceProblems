class Solution:
    def maxSlidingWindowBruteForce(self, nums: List[int], k: int) -> List[int]:
        ...
        #O(Nk) - time, O(N-K+1) for output array
        return [max(nums[i:i+k]) for i in range(len(nums - k + 1))]
    
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:
        #Bruteforce – O(Nk) time – two loops find max and add to result
#Heap of k – heap[0] to find the largest element, o(N log k)
        #O(k) for deque, O(N) time
