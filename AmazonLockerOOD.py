...
class Size(Enum):#standard locker sizes for simplicity sake
    SMALL = 1
    MEDIUM = 2
    LARGE = 3
    EXTRALARGE = 4
...
class PackageFactory:#avoid this because we don't have inheritance
    @staticmethod
...
class Locker:
    ...
    def __init__(self, locker_size):
        ...
        self.vacancy_status = True #or set it to occupied
...
class LockerManager:
    def __init__(self, lockers = []):
        self.lockers = lockers #list of lockers
        self.locker_map = {} #lockers with packages in map 
    ...
    def occupy_locker(self, package):
        ...
        for locker in self.get_lockers():
            if locker.get_vacancy_status():
                match = LockerManager.match_sizes(locker, package)#modularized
